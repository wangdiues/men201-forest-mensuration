// MEN201 grader — Cloud Functions (Node 20)
//
// ① onAttemptCreated     — grades auto-graded items, writes breakdowns,
//                          strengths/improvements, updates results + stats.
// ② onAttemptManualGraded — merges teacher-awarded marks into the final total.
//
// Only these functions (admin SDK) may write score/breakdown/stats fields;
// the security rules deny all clients those writes.
...
const GAP_THRESHOLD = 60;   // percent below which a level is flagged
...
function gradeQuestion(q, given) {
  switch (q.questionType) {
    ...
    case "matching": {
      // pairs[i] = {left, right}; the client renders right items in a
      // shuffled display order, but each <option value> is the ORIGINAL index
      // of the right item. givenPairs[i] = original index of the right item
      // chosen for left item i. Correct iff givenPairs[i] === i.
      const pairs = q.pairs || [];
...
function finishBreakdown(map) {
  const out = {};
  BLOOM_LEVELS.concat().forEach((lv) => {
    const e = map[lv];
    if (e && e.available > 0) {
      out[lv] = {
...
async function loadContext(attempt) {
  ...
  return { questions, assessment, sections };
}
...
exports.onAttemptCreated = onDocumentCreated("attempts/{attemptId}", async (event) => {
  const attempt = event.data.data();
  if (!attempt || attempt.status !== "submitted") return;

  const { questions, assessment, sections } = await loadContext(attempt);
...
exports.onAttemptManualGraded = onDocumentUpdated("attempts/{attemptId}", async (event) => {
  const before = event.data.before.data();
  const after = event.data.after.data();
  if (before.status !== "awaiting-manual" || after.status !== "complete") return;
  if (!Array.isArray(after.manualGrading) || !after.manualGrading.length) return;
  ...
  await bumpStats(after.assessmentId, after.userId, {
    perLevel,
    perTopic,
    perQ,
    percent,
...
async function upsertResult(uid, assessmentId, percent, bloom, submittedAt) {
  ...
  await db.runTransaction(async (t) => {
    ...
    t.set(ref, {
      ...
      avgPercent,
      lastPercent: percent,
      lastAttemptAt: submittedAt || FieldValue.serverTimestamp(),
      bloomProfile,
    });
  });
}
